...
"""
ROS 2 node to find a collision-free joint-space path using the Informed RRT* algorithm.

References:
[1]. Original paper: Informed RRT*: Optimal Sampling-based Path Planning Focused via Direct Sampling of an Admissible Ellipsoidal Heuristic
[2]. Implementation from Python Robotics repository: https://github.com/AtsushiSakai/PythonRobotics/blob/master/PathPlanning/InformedRRTStar/informed_rrt_star.py
...
from robot_3r_msgs.msg import JointWaypoint, JointSpacePath
...
import fcl
from robot_3r_msgs.msg import SceneObstacles, RigidBodyGeom
...
class InformedRRTStarPlanner(Node):
    class InformedRRTStar:
        ...
        # initialize InformedRRTStar with params; these are set when the parent ROS2 Node is initialized
                ...
                if self.outer.pl_alg=="rrt_star":
                    if self.outer.use_goal_biased_sampling:
                        if np.random.rand() > self.goal_sample_rate:
                            ...
                            return self.InformedRRTStarConfigNode(goal_cand)
                    else:
                        samp_q = []
                        for joint_limit in self.outer.joint_limits:
                            q_curr_joint = np.random.uniform(joint_limit[0], joint_limit[1])
                    ...
                    self.outer.get_logger().warning("Only the InformedRRTStar sampling-based planning algorithm is supported!")
        ...
        ####################################################################
        # TODO (Exercise 1): 
        # Compute self.b and self.c_min within this method 
        # using equations (3) and (4) from the manual
        ####################################################################
        def compute_ellipse_params(self):
            ...
            if hasattr(self, 'c_best') and self.c_best < float('inf'): # DO NOT EDIT THIS LINE
        ...
        def compute_ellipse_center(self):
            """Return the center of the ellipse (midpoint between start and end) as a column vector."""
            center = (np.array(self.start.q, dtype=float) + np.array(self.end.q, dtype=float)) / 2.0
            return center.reshape((-1, 1))

        def compute_rotation_matrix(self):
            """Compute rotation matrix C that aligns the first axis with the vector from start to goal.

            Uses SVD-based method generalized to N-dimensions as in the PythonRobotics implementation.
            Returns an (n x n) rotation matrix.
            """
            try:
                start = np.array(self.start.q, dtype=float)
                goal = np.array(self.end.q, dtype=float)
                diff = goal - start
                ...
                e1 = np.zeros((self.dimension, 1), dtype=float)
                e1[0, 0] = 1.0

                m = a1 @ e1.T  # outer product
                U, _, Vt = np.linalg.svd(m, full_matrices=True)
                # correction matrix to ensure a proper rotation
        ...
        def sample_unit_hypersphere(self):
            """Uniformly sample a point in the unit n-ball and return as column vector (n x 1)."""
            n = self.dimension
            ...
            sample = (x * r).reshape((n, 1))
            return sample

        ####################################################################
        # NOTE: Complete this method. 
        #   Do not change anything outside the TODO-marked area below.
        ####################################################################
        def informed_sample(self):
            """Sample from the informed (ellipsoidal) distribution in C-space.

            Returns a 1-D numpy array of length self.dimension.
            Falls back to uniform sampling when c_best is infinite.
            """
            # if no current solution, fall back to uniform random sampling
            if not hasattr(self, 'c_best') or self.c_best == float('inf'):
                samp_q = []
                for jl in self.outer.joint_limits:
                    samp_q.append(np.random.uniform(jl[0], jl[1]))
                return np.array(samp_q)

            # ensure c_min and b are computed
            self.compute_ellipse_params()
            c_min = getattr(self, 'c_min', None)
            b = getattr(self, 'b', None)

            if c_min is None or c_min == float('inf'):
                samp_q = []
                for jl in self.outer.joint_limits:
                    samp_q.append(np.random.uniform(jl[0], jl[1]))
                return np.array(samp_q)

            # compute ellipsoid center and rotation matrix
            self.q_center = self.compute_ellipse_center()
            self.R_align = self.compute_rotation_matrix()

            # ellipsoid radii formulae: major axis = c_best/2, other axes = sqrt(c_best^2 - c_min^2)/2
            if b is None:
                semi_minor = float(max(self.c_best ** 2 - c_min ** 2, 0.0) ** 0.5) / 2.0
            semi_minor = float(b)
            L_scale_vec = np.zeros(self.dimension)
            L_scale_vec[0] = self.c_best / 2.0
            L_scale_vec[1:] = semi_minor 
            self.L_scale = np.diag(L_scale_vec)

            max_attempts = 10
            low_limits = np.array([jl[0] for jl in self.outer.joint_limits])
            high_limits = np.array([jl[1] for jl in self.outer.joint_limits])
            for _ in range(max_attempts):
                self.q_unit = self.sample_unit_hypersphere()
                ####################################################################
                # TODO (Exercise 1): 
                # Compute self.q_ellipse in this block using equation (5) 
                # from the manual:
                #            ------------------|----------------
                #            Equation variable | class variable
                #            ------------------|----------------
                ...
                self.q_ellipse = 0.0 # MODIFY

                ####################################################################
                ####################################################################
                rnd = self.q_ellipse.flatten()
                # check if within limits
                if np.all(rnd >= low_limits) and np.all(rnd <= high_limits):
                    self.informed_sample_count = getattr(self, 'informed_sample_count', 0) + 1
                    return rnd

            # clip to joint limits as a last resort
            clipped = np.clip(rnd, low_limits, high_limits)
            self.informed_sample_count = getattr(self, 'informed_sample_count', 0) + 1
            return clipped
    
        def get_nearby_neighbors(self, x_new:InformedRRTStarConfigNode) -> List[InformedRRTStarConfigNode]:
            """
        ...
        def choose_best_parent(self, new_node:InformedRRTStarConfigNode, near_inds:List[int]) -> InformedRRTStarConfigNode:
        ...
        def steer(self, x_nearest:InformedRRTStarConfigNode, x_random:InformedRRTStarConfigNode):
            ...
            new_node = self.InformedRRTStarConfigNode(start.copy()) # initialize new node as start node
        ...
        def rewire(self, new_node:InformedRRTStarConfigNode, near_inds:List[int]):
            """
            ...
            for i in near_inds:
                ...
                if cand_new_node is None:
                    continue
                if not self.collision_free(cand_new_node, self.robot_geom, self.obstacle_geom, self.check_collision_param):
                    continue
                new_cost = new_node.cost + self.calc_new_cost(new_node, near_node)
                if new_cost < near_node.cost:
                    near_node.parent = new_node
                    near_node.cost = new_cost
                    self.rewire_count = getattr(self, 'rewire_count', 0) + 1
                    self.rewire_total = getattr(self, 'rewire_total', 0) + 1
                    if getattr(self.outer, 'verbose', False):
                        self.outer.get_logger().info(f"Rewired node {i}; new_cost={new_cost:.3f}; total_rewires={self.rewire_total}")
        ...
        def propagate_cost_to_leaves(self, parent_node:InformedRRTStarConfigNode):
            """
        ...
        def plan(self):
            ...
            # initialize best cost tracking for informed sampling
            if not hasattr(self, 'c_best'):
                self.c_best = float('inf')
            if not hasattr(self, 'best_path'):
                self.best_path = None
            if not hasattr(self, 'first_solution_cost'):
                self.first_solution_cost = None
            if not hasattr(self, 'informed_sample_count'):
                self.informed_sample_count = 0
            if not hasattr(self, 'c_best_updates'):
                self.c_best_updates = 0
            if not hasattr(self, '_debug_sample_logs'):
                self._debug_sample_logs = []
            if not hasattr(self, 'informed_total'):
                self.informed_total = 0
            if not hasattr(self, 'informed_accepted'):
                self.informed_accepted = 0
            if not hasattr(self, 'informed_rejected_collision'):
                self.informed_rejected_collision = 0
            if not hasattr(self, 'informed_rejected_steer'):
                self.informed_rejected_steer = 0
            ...
            for i in range(self.max_iter):
                ...
                # use informed sampling if we have a current solution; use uniform sampling otherwise
                rnd_arr = None
                try:
                    # before sampling, check whether informed sampling should be considered active
                    informed_active = (hasattr(self, 'c_best') and self.c_best < float('inf'))
                    rnd_arr = self.informed_sample()
                except Exception:
                    rnd_arr = None

                # if the sample came from the informed sampler while active, count it
                if rnd_arr is not None and informed_active:
                    self.informed_sample_count = getattr(self, 'informed_sample_count', 0) + 1
                    self.informed_total = getattr(self, 'informed_total', 0) + 1

                if rnd_arr is None:
                    rnd_node = self.sample_free()
                else:
                    rnd_node = self.InformedRRTStarConfigNode(np.array(rnd_arr))
                ...
                # count steer failures for informed samples
                if informed_active and rnd_arr is not None and new_node is None:
                    self.informed_rejected_steer = getattr(self, 'informed_rejected_steer', 0) + 1

                # check collision and process node
                collision_ok = False
                if new_node is not None:
                    collision_ok = self.collision_free(new_node, self.robot_geom, self.obstacle_geom, self.check_collision_param)
                    if informed_active and rnd_arr is not None and not collision_ok:
                        self.informed_rejected_collision = getattr(self, 'informed_rejected_collision', 0) + 1
                ...
                if new_node and collision_ok:
                    ...
                    if new_node:
                        ...
                        if informed_active and rnd_arr is not None:
                            self.informed_accepted = getattr(self, 'informed_accepted', 0) + 1

                        # check for path to goal and update current if better
                        last_index = self.find_best_goal_node()
                        if last_index is not None:
                            temp_path = self.generate_final_course(last_index)
                            temp_path_len = self.compute_path_cost(temp_path)
                           
                            # first solution found
                            if self.c_best == float('inf') and temp_path_len < float('inf'):
                                self.c_best = temp_path_len
                                self.best_path = temp_path
                                self.first_solution_cost = temp_path_len
                                self.compute_ellipse_params()
                                b = getattr(self, 'b', None)
                                c_min = getattr(self, 'c_min', None)
                                self.outer.get_logger().info(f"Informed sampling initialized: c_min={c_min:.3f}, b={b}")
                            
                            if temp_path_len < self.c_best: # improvement found
                                old = self.c_best
                                self.c_best = temp_path_len
                                self.best_path = temp_path
                                # recompute ellipse parameters for the new c_best
                                try:
                                    self.compute_ellipse_params()
                                except Exception:
                                    pass
                                self.c_best_updates = getattr(self, 'c_best_updates', 0) + 1
                                self.outer.get_logger().info(f"Updated c_best: {old:.3f} -> {self.c_best:.3f}")

                # early exit if caller requested full RRT* semantics
                if (not self.outer.rrts_search_until_max_iter) and self.best_path is not None and self.c_best < float('inf'):
                    break
            if self.outer.verbose:
            ...
            if self.outer.verbose:
                try:
                    informed_total = getattr(self, 'informed_total', 0)
                    informed_accepted = getattr(self, 'informed_accepted', 0)
                    informed_rej_col = getattr(self, 'informed_rejected_collision', 0)
                    informed_rej_steer = getattr(self, 'informed_rejected_steer', 0)
                    self.outer.get_logger().info(
                        f"Informed sampling summary: informed_samples={getattr(self,'informed_sample_count',0)}, total_tried={informed_total}, accepted={informed_accepted}, rejected_collision={informed_rej_col}, rejected_steer={informed_rej_steer}, c_best_updates={getattr(self,'c_best_updates',0)}"
                    )
                    if self._debug_sample_logs:
                        self.outer.get_logger().info(f"Example informed samples (up to 5): {self._debug_sample_logs}")
                except Exception:
                    pass
        ...
        def collision_free(self,
                    candidate_node: InformedRRTStarConfigNode,
            ...
            nodes_to_check = []
            try:
                nodes_to_check = [InformedRRTStarPlanner.InformedRRTStar.InformedRRTStarConfigNode(node) for node in candidate_node.path_q]
            except Exception as e:
        ...
        @staticmethod
        def calc_distance_and_angle(from_node:InformedRRTStarConfigNode, to_node:InformedRRTStarConfigNode):
        ...
        def compute_path_cost(path: List[np.ndarray]) -> float:
            ...
            return np.sum(np.linalg.norm(path_arr[1:] - path_arr[:-1], axis=1))
    ...
    def __init__(self, node_name:str="informed_rrt_star", queue_size=10):
        ...
        # InformedRRTStar-specific params
        self.declare_parameter(
            "rrts_expand_dist",
            value=0.3,
        ...
        ######################################################################################################################
        # subscriber for robot state (store handle so we can unsubscribe on stop)
        ...
        # Initialize attributes for tracking first and final solutions
        self.first_solution_path = None
        self.first_solution_cost = None
        self.final_solution_path = None
        self.final_solution_cost = None
    ...
    def compute_plan(self, msg:JointState):
        ...
        # if we've already reached the max, planning has permanently failed
        if getattr(self, 'planning_attempts', 0) >= getattr(self, 'max_planning_attempts', 0):
            if not getattr(self, 'planning_failed', False):
                ...
                try:
                    if hasattr(self, 'joint_state_sub') and self.joint_state_sub is not None:
                        ...
                        self.get_logger().info('Max. attempts reached; unsubscribing from joint_states topic.')
                except Exception:
                    pass
                if self.stop_on_failure:
                    self.get_logger().info('Scheduling node shutdown due to exceeding max_planning_attempts.')
        ...
        if self.start_config is not None and self.obstacle_geom is not None and self.robot_geom is not None:
            # create the InformedRRTStar instance once and reuse it across callbacks so informed
            # sampling state (c_best, counters) is preserved between calls
            if self.rrt_star is None:
                self.rrt_star = self.InformedRRTStar(
                    self,
                ...
                # update start/goal and scene info for the existing planner
                try:
                    self.rrt_star.start.q = np.array(self.start_config, dtype=float)
                    self.rrt_star.start.path_q = [np.array(self.start_config, dtype=float)]
                    self.rrt_star.end.q = np.array(self.goal_config, dtype=float)
                    self.rrt_star.end.path_q = [np.array(self.goal_config, dtype=float)]
                    self.rrt_star.robot_geom = self.robot_geom
                    self.rrt_star.obstacle_geom = self.obstacle_geom
                except Exception:
                    pass
            ...
            if computed_path is None:
                ...
                # if we already have a previously computed valid path, keep it but still honor max attempts
                prev_exists = (self.computed_path is not None) or (getattr(self.rrt_star, 'best_path', None) is not None)
                if prev_exists:
                    self.get_logger().warning("New planning attempt failed but a previous valid plan exists; keeping previous solution.")

                # check if we've exceeded maximum attempts and handle failure (honor even when previous solution exists)
                if self.planning_attempts >= self.max_planning_attempts:
                    ...
                    # unsubscribe from joint states to prevent further planning callbacks
                    try:
                        if hasattr(self, 'joint_state_sub') and self.joint_state_sub is not None:
                            self.destroy_subscription(self.joint_state_sub)
                            self.joint_state_sub = None
                            self.get_logger().info("Unsubscribed from /joint_states to stop re-planning.")
                    except Exception:
                        pass
                    if self.stop_on_failure:
                        # use a timer to shutdown gracefully so logs flush
                        self.get_logger().info("Scheduling node shutdown due to repeated planning failures.")
                        self.create_timer(1.0, lambda: rclpy.shutdown())
                        
            else:
                # store first solution when found
                init_cost = getattr(self.rrt_star, 'first_solution_cost', None)
                final_cost = getattr(self.rrt_star, 'c_best', None)
                
                if init_cost is not None and self.first_solution_path is None:
                    self.first_solution_path = computed_path.copy()
                    self.first_solution_cost = init_cost
                
                # update final solution
                if final_cost is not None and final_cost < float('inf'):
                    self.final_solution_path = computed_path.copy()
                    self.final_solution_cost = final_cost
                else:
                    self.final_solution_path = computed_path.copy()
                    self.final_solution_cost = self.rrt_star.compute_path_cost(computed_path)
                
                # only publish and display waypoints when planning is complete
                planning_complete = (self.planning_attempts >= self.max_planning_attempts) or self.stop_if_plan_found
                
                if planning_complete:
                    # print comparison between first and final solutions
                    self.get_logger().info("=" * 80)
                    self.get_logger().info("PLANNING COMPLETE - SOLUTION COMPARISON")
                    self.get_logger().info("=" * 80)
                    
                    if self.first_solution_cost is not None:
                        self.get_logger().info(f"First RRT* solution cost: {self.first_solution_cost:.3f}")
                        self.get_logger().info(f"First RRT* waypoint count: ({len(self.first_solution_path)}):")
                    
                    if self.final_solution_cost is not None:
                        self.get_logger().info("-" * 80)
                        improvement = ""
                        if self.first_solution_cost is not None:
                            improvement = f" \033[92m(improvement: {self.first_solution_cost - self.final_solution_cost:.3f})\033[0m"
                        self.get_logger().info(f"\033[92mFinal Informed RRT* solution cost: {self.final_solution_cost:.3f}{improvement}\033[0m")
                        self.get_logger().info(f"\033[92mFinal Informed RRT* waypoint count: ({len(self.final_solution_path)}):\033[0m")
                        for i, q in enumerate(self.final_solution_path):
                            self.get_logger().info(f"  \033[92m[{i}] {np.round(q, 3)}\033[0m")
                    ...
                    for q in self.final_solution_path:
                        waypoint = JointWaypoint()
                        waypoint.positions = np.array(q).tolist()
                        plan_msg.waypoints.append(waypoint)
                    
                    self.get_logger().info(f"Publishing final path with {len(plan_msg.waypoints)} waypoints")
                    self.plan_pub.publish(plan_msg)
                    self.computed_path = self.final_solution_path

                    # stop planning
                    self.planning_done = True
                    if self.stop_if_plan_found:
                        self.get_logger().info("Final plan found; stopping planning node")
                        try:
                            self.create_timer(1.0, lambda: rclpy.shutdown())
                        except Exception:
                            pass
                    ...
                    # log progress only
                    if init_cost is not None and self.verbose:
                        self.get_logger().info(f"Found initial RRT* path (first solution) with cost: {init_cost:.3f}")
                    if final_cost is not None and final_cost < float('inf'):
                        self.get_logger().info(f"Current best Informed RRT* cost: {final_cost:.3f}")
                    else:
...
def main(args=None):
    ...
    informed_rrt_star_node = InformedRRTStarPlanner()
    try:
        rclpy.spin(informed_rrt_star_node)
    except KeyboardInterrupt:
        pass
    except SystemExit:
        pass
    finally:
        informed_rrt_star_node.destroy_node()
        rclpy.shutdown()
