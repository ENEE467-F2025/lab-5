#!/usr/bin/env python3
"""
Example of moving to a pose goal using the UR3e robot.
- ros2 run lab_moveit_scripts ur3_pose_goal.py --ros-args -p position:="[0.25, 0.0, 1.0]" -p quat_xyzw:="[0.0, 0.0, 0.0, 1.0]" -p cartesian:=False
- ros2 run lab_moveit_scripts ur3_pose_goal.py --ros-args -p position:="[0.25, 0.0, 1.0]" -p quat_xyzw:="[0.0, 0.0, 0.0, 1.0]" -p cartesian:=False -p synchronous:=False -p cancel_after_secs:=1.0
"""
...
def main():
    ...
    # Create MoveIt2 interface for UR3e
    moveit2 = MoveIt2(
        node=node,
        joint_names=ur.joint_names(),  # UR3e joint names
        base_link_name=ur.base_link_name(),  # UR3e base link name
        end_effector_name=ur.end_effector_name(),  # UR3e end effector name
        group_name=ur.MOVE_GROUP_ARM,  # UR3e group name for the arm
