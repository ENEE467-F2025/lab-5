#!/usr/bin/env python3
"""
Example of moving the 3R robot to a joint configuration.
- ros2 run robot_3r_moveit_scripts joint_goal.py --ros-args -p joint_positions:="[1.57, -1.57, 0.0]"
- ros2 run robot_3r_moveit_scripts joint_goal.py --ros-args -p joint_positions:="[1.57, -1.57, 0.0]" -p synchronous:=False -p cancel_after_secs:=1.0
- ros2 run robot_3r_moveit_scripts joint_goal.py --ros-args -p joint_positions:="[1.57, -1.57, 0.0]" -p synchronous:=False -p cancel_after_secs:=0.0
"""
...
def main():
    ...
    node.declare_parameter("cancel_after_secs", 0.0) # If non-positive, don't cancel. Only used if synchronous is False
    node.declare_parameter("planner_id", "RRTConnectkConfigDefault")     # Planner ID
